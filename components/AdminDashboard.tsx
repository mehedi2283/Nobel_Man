...
const SortableProjectItem = React.memo(({ 
    ...
    return (
        <Reorder.Item
            ...
            onDragEnd={onDragEnd}
            layout
            className="p-4 rounded-sm border border-gray-200 flex flex-col md:flex-row items-center gap-4 group cursor-grab active:cursor-grabbing relative overflow-hidden"
        >
            <div className="flex items-center gap-4 w-full md:w-auto pointer-events-none">
                {/* Updated Count Circle: Animates on Drag */}
                <motion.div 
                    variants={countVariants}
                    className="w-8 h-8 rounded-sm border shadow-sm flex items-center justify-center text-sm font-bold transition-colors"
                >
                    {index + 1}
                </motion.div>
            ...
            <div className="flex items-center gap-2 ml-auto w-full md:w-auto justify-end">
                ...
                <button onPointerDown={(e) => e.stopPropagation()} onClick={onOpenComments} className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-sm transition-colors flex items-center gap-2 text-sm font-medium px-2 md:px-3"><MessageSquare size={16} /></button>
                <button onPointerDown={(e) => e.stopPropagation()} onClick={onEdit} className="p-2 text-blue-600 hover:bg-blue-50 rounded-sm transition-colors flex items-center gap-2 text-sm font-medium px-2 md:px-3"><Edit2 size={16} /> <span className="hidden sm:inline">Edit</span></button>
                <button onPointerDown={(e) => e.stopPropagation()} onClick={onDelete} className="p-2 text-red-600 hover:bg-red-50 rounded-sm transition-colors flex items-center gap-2 text-sm font-medium px-2 md:px-3"><Trash2 size={16} /> <span className="hidden sm:inline">Delete</span></button>
            </div>
        </Reorder.Item>
    );
});
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ projects, onSaveProject, onDeleteProject, onReorderProjects, onProjectUpdate, onProfileUpdate, onLogout, onRefreshRequests, homeLogo }) => {
  ...
  const unreadChatLogsCount = useMemo(() => {
      // Filter distinct conversation starts or just raw logs? 
      // For simplicity, count raw unread logs where role is user? 
      // Usually admins want to see new conversations. 
      // Current system stores logs individually. 
      // Let's count unread user messages.
  ...
  const handleOpenComments = (project: Project) => {
      ...
      // Mark all comments for this project as read when opened? 
      // Or let user click individually. Prompt says "when seen it will be gone".
      // Let's implement individual click to mark read inside the modal.
  };
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        days.push(
            <button
                ...
                className={`w-8 h-8 sm:w-9 sm:h-9 rounded-sm flex items-center justify-center text-sm font-medium transition-all duration-200 select-none cursor-pointer
  ...
  const getTabClass = (tabName: string) => {
    ...
    return `flex items-center gap-3 px-4 py-3 rounded-sm transition-all border ${
      isActive 
      ...
      return (
        <div className="space-y-2 relative" ref={containerRef}>
          ...
          <button
            ...
            className="w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-sm focus:outline-none focus:border-black transition-all font-medium text-left flex justify-between items-center group hover:border-gray-400"
          >
          ...
          <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-sm shadow-xl max-h-60 overflow-y-auto z-50 scrollbar-hide"
                >
                    {years.map(y => (
                        <button
                            key={y}
                            ...
                            className={`w-full text-left px-4 py-3 text-sm transition-colors hover:bg-gray-50 first:rounded-t-sm last:rounded-b-sm
        ...
        <div className="container mx-auto px-6 h-16 flex items-center justify-between">
            <div className="flex items-center gap-3">
                {homeLogo ? (
                    ...
                    <div className="w-8 h-8 bg-black rounded-sm flex items-center justify-center text-white font-bold">N</div>
                )}
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="hidden md:flex w-64 flex-col gap-2 shrink-0 sticky top-24">
                ...
                >
                    ...
                    Inbox
                    {totalUnreadInbox > 0 && (
                         <span className="ml-auto bg-gray-100 text-gray-900 text-xs px-2 py-0.5 rounded-sm font-bold">{totalUnreadInbox}</span>
                    )}
                </button>
                <button 
                ...
                >
                    <LayoutGrid size={20} />
                    All Projects
                    <span className="ml-auto bg-gray-100 text-gray-900 text-xs px-2 py-0.5 rounded-sm font-bold">{projects.length}</span>
                </button>
                <button 
                ...
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === 'overview' && (
                        <motion.div key="overview" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-8">
                           ...
                           <div className="bg-white rounded-sm border border-gray-200 p-6 md:p-8 shadow-sm">
                               <div className="flex items-center gap-3 mb-8"><div className="w-10 h-10 rounded-sm bg-black flex items-center justify-center text-white"><Sparkles size={20} /></div><div><h3 className="text-lg font-bold text-gray-900">Recent Activity</h3><p className="text-sm text-gray-500">Latest comments and incoming messages</p></div></div>
                               <div className="space-y-6">
                                   {recentActivity.length > 0 ? (
                                       recentActivity.map((item) => (
                                           <div 
                                                key={item.id} 
                                                className={`flex gap-4 items-start group p-2 rounded-sm transition-colors ${!item.read ? 'bg-blue-50/50 cursor-pointer' : ''}`}
                                                onClick={() => {
                                                    if (!item.read) {
                                           ...
                                           >
                                               ...
                                               <div className="flex-1 min-w-0">
                                                   ...
                                                   <ExpandableMessage 
                                                        text={item.text} 
                                                        className={`p-4 rounded-sm text-sm text-gray-600 transition-colors mt-2 ${!item.read ? 'bg-white border border-blue-100' : 'bg-gray-50 group-hover:bg-gray-100'}`}
                                                   />
                                               </div>
                                           </div>
                                       ))
                    ...
                    {activeTab === 'inbox' && (
                        <motion.div key="inbox" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white rounded-sm shadow-lg border border-gray-200 p-8 max-w-4xl mx-auto flex flex-col min-h-[600px]">
                           <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                                <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-3">Inbox<span className="text-sm font-normal text-gray-500 bg-gray-100 px-3 py-1 rounded-sm">{processedInboxData.length} items</span></h2>
                                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                    <div className="bg-gray-100 p-1 rounded-sm flex items-center relative gap-1"><button onClick={() => setInboxTab('contact')} className={`relative flex-1 px-8 py-2 text-sm font-bold transition-colors z-10 rounded-sm whitespace-nowrap min-w-[120px] ${inboxTab === 'contact' ? 'text-black' : 'text-gray-500'}`}>Contact Form{inboxTab === 'contact' && (<motion.div layoutId="inbox-tab" className="absolute inset-0 bg-white rounded-sm shadow-sm -z-10 border border-black/5" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />)}</button><button onClick={() => setInboxTab('chat')} className={`relative flex-1 px-8 py-2 text-sm font-bold transition-colors z-10 rounded-sm whitespace-nowrap min-w-[120px] ${inboxTab === 'chat' ? 'text-black' : 'text-gray-500'}`}>Chatbot{inboxTab === 'chat' && (<motion.div layoutId="inbox-tab" className="absolute inset-0 bg-white rounded-sm shadow-sm -z-10 border border-black/5" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />)}</button></div>
                                    <div className="relative">
                                        <button 
                                            className={`relative cursor-pointer border rounded-sm px-5 py-2 flex items-center gap-3 transition-all h-[40px] ${dateFilter ? 'bg-black text-white border-black' : 'bg-white border-gray-200 hover:border-gray-400 text-gray-600'}`} 
                                        ...
                                        >
                                            ...
                                            {dateFilter && (<span onClick={(e) => { e.stopPropagation(); setDateFilter(''); }} className="ml-2 w-5 h-5 bg-white text-black rounded-sm flex items-center justify-center hover:bg-gray-200 transition-colors"><X size={12} /></span>)}
                                        </button>
                                        ...
                                        <AnimatePresence>
                                            {showDatePicker && (
                                                <motion.div 
                                                    ...
                                                    className="absolute left-0 sm:left-auto sm:right-0 top-full mt-2 bg-white rounded-sm shadow-xl border border-gray-200 p-4 z-50 w-[280px] sm:w-[320px]"
                                                >
                                                    ...
                                                    <div className="flex items-center justify-between mb-4 px-1">
                                                        <div className="flex gap-1">
                                                            <button onClick={handlePrevYear} className="p-1.5 hover:bg-gray-100 rounded-sm text-gray-400 hover:text-black transition-colors" title="Previous Year">
                                                                <ChevronsLeft size={18} />
                                                            </button>
                                                            <button onClick={handlePrevMonth} className="p-1.5 hover:bg-gray-100 rounded-sm text-gray-400 hover:text-black transition-colors" title="Previous Month">
                                                                <ChevronLeft size={18} />
                                                            </button>
                                                        </div>
                                                        ...
                                                        <div className="flex gap-1">
                                                            <button onClick={handleNextMonth} className="p-1.5 hover:bg-gray-100 rounded-sm text-gray-400 hover:text-black transition-colors" title="Next Month">
                                                                <ChevronRight size={18} />
                                                            </button>
                                                            <button onClick={handleNextYear} className="p-1.5 hover:bg-gray-100 rounded-sm text-gray-400 hover:text-black transition-colors" title="Next Year">
                    ...
                    {showCommentsModal && selectedProjectForComments && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowCommentsModal(false)}>
                            <motion.div className="bg-white w-full max-w-2xl rounded-sm shadow-2xl overflow-hidden max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                                    ...
                                    <button onClick={() => setShowCommentsModal(false)} className="p-2 hover:bg-gray-200 rounded-sm transition-colors"><X size={20} /></button>
                                </div>
                                    ...
                                    {!selectedProjectForComments.comments || selectedProjectForComments.comments.length === 0 ? (
                                        ...
                                        <div className="space-y-4">
                                            {(selectedProjectForComments.comments || []).slice().reverse().map((comment: any, idx: number) => (
                                                <div 
                                                    key={idx} 
                                                    className={`flex gap-4 p-4 border rounded-sm transition-colors group items-center ${!comment.read ? 'bg-blue-50 border-blue-200 cursor-pointer' : 'border-gray-100 hover:bg-gray-50'}`}
                                                    onClick={() => handleMarkCommentRead(selectedProjectForComments.id, comment._id || comment.id)}
                                                >
                                                    <div className="w-10 h-10 rounded-sm bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-sm shrink-0">{comment.author.charAt(0).toUpperCase()}</div>
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex justify-between items-start mb-1">
                                                            <h4 className={`font-bold text-sm ${!comment.read ? 'text-black' : 'text-gray-900'}`}>
                                                                {comment.author}
                                                                {!comment.read && <span className="ml-2 w-2 h-2 rounded-full bg-blue-500 inline-block"></span>}
                                                            </h4>
                                                            <span className="text-xs text-gray-400">{new Date(comment.createdAt).toLocaleDateString()}</span>
                                                        </div>
                                                        <p className="text-sm text-gray-600 leading-relaxed">{comment.text}</p>
                                                    </div>
                                                    <button onClick={(e) => { e.stopPropagation(); requestDeleteComment(comment._id || comment.id); }} className="p-3 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-sm transition-all" title="Delete Comment"><Trash2 size={18} /></button>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                </div>
                                <div className="p-4 border-t border-gray-100 bg-gray-50 text-right"><button onClick={() => setShowCommentsModal(false)} className="px-6 py-2 bg-black text-white rounded-sm hover:bg-gray-800 transition-colors text-sm font-medium">Done</button></div>
                            </motion.div>
                        </motion.div>
                    )}
                    ...
                    {activeTab === 'form' && (
                        <motion.div key="form" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white rounded-sm shadow-lg border border-gray-200 p-8 max-w-3xl mx-auto">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-8">
                                ...
                                <div className="space-y-2"><label className="text-sm font-bold text-gray-700 block">Description</label><textarea name="description" value={formData.description} onChange={handleInputChange} rows={5} className="w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-sm focus:outline-none focus:border-black resize-none" placeholder="Detailed project description..."/></div>
                                <div className="pt-6 border-t border-gray-100 flex justify-end gap-3 sticky bottom-0 bg-white py-4 z-10"><button type="button" onClick={() => { setActiveTab('list'); resetForm(); }} className="px-6 py-3 text-gray-600 font-medium hover:bg-gray-100 rounded-sm">Cancel</button><button type="submit" className="px-8 py-3 bg-black text-white font-medium rounded-sm hover:bg-gray-800 flex items-center gap-2"><Save size={18} />{editingId ? 'Update Project' : 'Save Project'}</button></div>
                            </form>
                        </motion.div>
                    )}

                    {activeTab === 'logos' && (
                        <motion.div key="logos" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white rounded-sm shadow-lg border border-gray-200 p-8 max-w-4xl mx-auto">
                            <h2 className="text-2xl font-bold text-gray-900 mb-6">Manage Client Logos</h2>
                            <div className="bg-gray-50 p-6 rounded-sm mb-8 border border-gray-200"><h3 className="font-bold text-lg mb-4">Add New Logo</h3><form onSubmit={handleAddLogo} className="flex flex-col md:flex-row gap-4 items-end"><div className="flex-1 w-full space-y-2"><label className="text-sm font-bold text-gray-700">Client Name <span className="text-gray-400 font-normal">(Optional)</span></label><input type="text" value={newLogoName} onChange={(e) => setNewLogoName(e.target.value)} placeholder="e.g. Nike" className="w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-sm focus:outline-none focus:border-black" /></div><div className="flex-[2] w-full space-y-2"><label className="text-sm font-bold text-gray-700">Logo Image URL</label><input type="url" value={newLogoUrl} onChange={(e) => setNewLogoUrl(e.target.value)} placeholder="https://..." className="w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-sm focus:outline-none focus:border-black" required /></div><button type="submit" disabled={isAddingLogo} className="px-6 py-3 bg-black text-white font-medium rounded-sm hover:bg-gray-800 disabled:opacity-50 h-[50px]">{isAddingLogo ? 'Adding...' : 'Add Logo'}</button></form></div>
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-gray-100"><div className="flex items-center justify-between w-full sm:w-auto gap-4"><span className="text-sm font-bold text-gray-500 whitespace-nowrap">{logos.length} Logos</span><button onClick={toggleAllLogos} className="text-sm font-medium text-gray-900 hover:text-black flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-gray-100 transition-colors"><div className={`w-5 h-5 border-2 rounded flex items-center justify-center transition-colors ${selectedLogoIds.size === logos.length && logos.length > 0 ? 'bg-black border-black text-white' : 'border-gray-300 bg-white'}`}>{selectedLogoIds.size === logos.length && logos.length > 0 && <Check size={12} strokeWidth={3} />}</div>Select All</button></div><AnimatePresence>{selectedLogoIds.size > 0 && (<motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} onClick={() => setShowBulkDeleteConfirm(true)} className="w-full sm:w-auto text-sm font-bold text-red-600 bg-red-50 hover:bg-red-100 flex items-center justify-center gap-2 px-6 py-3 rounded-sm transition-colors shadow-sm"><Trash2 size={18} /> Delete Selected ({selectedLogoIds.size})</motion.button>)}</AnimatePresence></div>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">{logos.map((logo) => { const id = logo._id || logo.id || ''; const isSelected = selectedLogoIds.has(id); return (<div key={id} onClick={() => toggleLogoSelection(id)} className={`relative group border rounded-sm p-4 flex flex-col items-center justify-center h-40 transition-all cursor-pointer select-none ${isSelected ? 'border-black ring-1 ring-black bg-gray-50/50' : 'border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm'}`}><div className="absolute top-3 left-3 z-10"><div className={`w-6 h-6 rounded-sm border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-black border-black text-white' : 'bg-white border-gray-200 text-transparent group-hover:border-gray-300'}`}><Check size={14} strokeWidth={3} /></div></div><button onClick={(e) => { e.stopPropagation(); setLogoToDelete(id); }} className={`absolute top-2 right-2 p-2.5 rounded-sm transition-all z-20 ${isSelected || 'md:opacity-0 md:group-hover:opacity-100'} ${isSelected ? 'bg-white text-red-500 shadow-sm' : 'bg-red-50 text-red-500'} hover:bg-red-100 hover:scale-110 active:scale-95`} title="Delete Logo"><Trash2 size={16} /></button><div className="flex-1 w-full flex items-center justify-center p-2"><img src={logo.url} alt={logo.name} className="max-w-full max-h-full object-contain pointer-events-none" loading="lazy" /></div>{logo.name && (<div className="w-full text-center"><span className="text-xs font-medium text-gray-500 truncate block px-2 py-1 bg-gray-100/50 rounded-sm">{logo.name}</span></div>)}</div>); })}</div>
                    ...
                    {activeTab === 'profile' && (
                        <motion.div key="profile" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="max-w-4xl mx-auto space-y-8 pb-12">
                            ...
                            <section className="bg-white rounded-sm shadow-sm border border-gray-200 p-6 md:p-8">
                                ...
                                <div className="space-y-2 mb-6"><label className="text-sm font-bold text-gray-700 block">Short Bio</label><textarea name="bio" value={profileData.bio} onChange={handleProfileChange} rows={4} className="w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-sm focus:outline-none focus:border-black resize-none" placeholder="I’m a UX/UI Designer..."/></div>
                            </section>

                            <section className="bg-white rounded-sm shadow-sm border border-gray-200 p-6 md:p-8">
                                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100"><div className="w-10 h-10 rounded-sm bg-blue-50 flex items-center justify-center text-blue-600"><LayoutGrid size={20} /></div><div><h3 className="font-bold text-lg text-gray-900">Hero Section</h3><p className="text-sm text-gray-500">Main landing area details</p></div></div>
                            ...
                            <section className="bg-white rounded-sm shadow-sm border border-gray-200 p-6 md:p-8">
                                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100"><div className="w-10 h-10 rounded-sm bg-green-50 flex items-center justify-center text-green-600"><LinkIcon size={20} /></div><div><h3 className="font-bold text-lg text-gray-900">Contact & Socials</h3><p className="text-sm text-gray-500">Links and contact info</p></div></div>
...
const StatCard = ({ title, value, icon }: { title: string; value: number; icon: React.ReactNode }) => (
    <div className="bg-white p-6 rounded-sm shadow-sm border border-gray-200 flex items-center justify-between">
        ...
        <div className="w-12 h-12 rounded-sm bg-gray-50 flex items-center justify-center">
            {icon}
        </div>
    </div>
);
...
const Input = ({ label, name, value, onChange, placeholder }: any) => (
    <div className="space-y-2">
        ...
        <input type="text" name={name} value={value} onChange={onChange} placeholder={placeholder} className="w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-sm focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition-all font-medium"/>
    </div>
);
        ...
        <div className="space-y-2">
            ...
            <div className="flex gap-3 items-center">
                <input 
                    ...
                    className="flex-1 w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-sm focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition-all font-medium"
                />
                <AnimatePresence>
                    {value && !error && (
                        <motion.div 
                            ...
                            className="shrink-0"
                        >
                            <div className="relative w-16 h-[46px] bg-white rounded-sm border border-gray-200 overflow-hidden flex items-center justify-center shadow-sm">
